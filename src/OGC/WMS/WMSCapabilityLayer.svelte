<script>
    ...
    function btnAddLayerClicked() {
        ...
        let source = leafletWMS.source(url, {'transparent': true, 'format': 'image/png'}); 
...
<div class="flex mt-1 relative text-gray-700">
    <p class="flex-grow text-grey-darkest hover:bg-red truncate text-left text-xs" title="{wmsLayerCapability.title()}">{wmsLayerCapability.title()}</p>
